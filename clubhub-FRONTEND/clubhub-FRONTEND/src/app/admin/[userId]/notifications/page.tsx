...
const AdminDashboard = () => {
  const { userId } = useParams(); // Updated to userId
  ...
  console.log("User ID from URL:", userId); // Debug: Check if userId is being set

  useEffect(() => {
    const fetchNotifications = async () => {
      if (!userId) return; // Exit if userId is not defined
      try {
        console.log(`Attempting to fetch notifications for userId: ${userId}`); // Debug
